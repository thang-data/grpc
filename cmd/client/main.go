...
func createLaptop(laptopClient pb.LaptopServiceClient, laptop *pb.Laptop) {
	...
	log.Printf("creating laptop with id %s", res.Id)
}
...
func rateLaptop(laptopClient pb.LaptopServiceClient, laptopIDs [] string, scores []float64) error {
	...
	go func() {
		for {
			...
			if err != nil {
				waitResponse <- fmt.Errorf("cannot read stream response: %v", err)
				return
			}

			log.Printf("recived response from server ", res)
		}
	}()
	// send request
	...
	for i, laptopID := range laptopIDs {
		...
		err := stream.Send(req)
		if err != nil {
			return fmt.Errorf("error sending request: %v - %v", err, stream.RecvMsg(nil))
		}
...
func uploadImage(laptopClient pb.LaptopServiceClient, laptopID string, imagePath string){
	...
	for {
		...
		err = stream.Send(req)
		if err != nil {
			log.Fatal("cannot send chunk to stream ", err)
		}
	}
	...
	b := namegenerator.NewNameGenerator(int64(len(res.GetId())))
	name := b.Generate()
	log.Printf("image upload with id: %s, %d", name, res.GetSize())
}
...
func main() {
	...
	if err != nil {
		log.Fatal("cannot dial server address", err)
	}
